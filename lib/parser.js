...
export class ParserManager {
  ...
  async init() {
    if (this.project.type === 'ableton-project') {
      let projectFiles = this.getProjectFile();
      if (projectFiles.length < 1) {
        throw new Error('Unable to find project file');
      }
      this.parser = await abletonParseFile(projectFiles[0]);
      this.watcher = ProjectWatcher.watch(this.project.id, async path => {
        if (extname(path) === '.als') {
          await ParserManager.resetInstance(this.project.id);
        }
